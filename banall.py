...
class BanAllModule(loader.Module):
    """Модуль для массового бана пользователей в канале/чате, кроме админов и ботов."""
    
    strings = {
        "name": "BanAllModule",
        "no_channel_id": "<b>❌ Ошибка:</b> Укажите юзер без @ канала/чата.",
        ...
        "invalid_channel_id": "<b>❌ Ошибка:</b> тег канала/чата должен быть буквеный."
    }
    ...
    async def client_ready(self, client, db):
        self.client = client
        self.db = db
        logger.info("BanAllModule готов к работе!")

    @loader.owner
    async def banallcmd(self, message: Message):
        """[ID канала/чата] - Бан всех пользователей в канале/чате"""
        if self.running:
